...
public interface FileEntityRepository extends EntityRepository<File> {
    ...
    @Query(value = "SELECT f.filename, f.link, f.id, f.created, f.updated, f.deleted, f.user_uuid FROM files f JOIN order_files off ON off.file_id = f.id WHERE off.order_id = :orderId", nativeQuery = true)
